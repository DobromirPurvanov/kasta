...
## Контактна форма (12.08.2026)

`src/components/ContactForm.tsx` + Pages Function `functions/api/contact.ts`.
Дотогава сайтът имаше само `mailto:` връзки.

**Формата няма да праща, докато не се зададат тайните.** Без тях функцията връща
`503 not_configured`, а формата показва резервните контакти вместо да мълчи.

В Cloudflare → Pages → `kasta` → Settings → **Variables and secrets**:

| Име | Тип | Стойност |
|---|---|---|
| `SMTP_HOST` | Text | напр. `mail.kastaventures.com` |
| `SMTP_PORT` | Text | `465` |
| `SMTP_USER` | Text | пълният адрес, от който се праща |
| `SMTP_PASS` | **Secret** | паролата на кутията |
| `MAIL_TO` | Text | по избор, по подразбиране `office@kastaventures.com` |

`SMTP_PASS` задължително като **Secret**, не Text — иначе се чете от дашборда.

### Защо кодът изглежда така

Cloudflare Workers **нямат** SMTP клиент — само суров TCP сокет
(`cloudflare:sockets`), затова протоколът е изписан на ръка. Ползва се
**имплицитен TLS на порт 465**, а не STARTTLS: спестява договарянето, което е
най-крехката част. Ако SuperHosting не приемат 465, алтернативата е 587 със
STARTTLS, но тогава кодът трябва да извика `startTls()` след `EHLO`.

Дреболии, които мълчаливо чупят поща и затова са нарочно решени:
- заглавията с кирилица се кодират по RFC 2047, иначе се получава каша;
- точка в началото на ред приключва `DATA`, затова се удвоява (RFC 5321);
- четенето изчаква пълен отговор (`250 ` а не `250-`), иначе многоредовият
  отговор на `EHLO` разминава целия диалог.

Капанът за ботове е скрито поле `company` — **не** е поле за бележка, защото
браузърите го попълват автоматично и режат истински запитвания. При попълнено
поле връщаме `200`, за да не подскажем на бота, че е разкрит.

`tsconfig.functions.json` е отделен, защото `tsc -b` на приложението не покрива
`functions/`. Проверява се с `npx tsc -p tsconfig.functions.json`.
