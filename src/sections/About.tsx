...
export default function About() {
  ...
  return (
    <section id="about" ref={sectionRef} className="dark section-dark-gradient relative section-pad overflow-hidden">
      ...
      <div className="section-shell relative">
        <div className="about-reveal grid lg:grid-cols-[minmax(0,1fr)_minmax(320px,520px)] gap-6 lg:gap-16 items-end mb-10 sm:mb-14">
          <div>
            ...
            <h2 className="text-display uppercase text-fg text-[clamp(2.55rem,6vw,5.25rem)]">
              {isBg ? <>Глобална машина.<br /><span className="text-[var(--accent-text)]">Локална грижа.</span></> : <>Global machine.<br /><span className="text-[var(--accent-text)]">Local care.</span></>}
            </h2>
          </div>
          <p className="text-[15px] sm:text-[16px] text-[var(--text-secondary)] leading-[1.8] lg:pb-1">
            {isBg
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-4 sm:gap-5">
          ...
          <div className="about-reveal lg:col-span-5 surface-card relative overflow-hidden rounded-[1.5rem] sm:rounded-[2rem] min-h-[250px] border-t-2 !border-t-[var(--accent)] p-6 sm:p-8 text-fg">
            <div className="absolute -right-8 -bottom-14 text-[12rem] font-extrabold leading-none text-[rgb(var(--accent-rgb)/0.08)]" aria-hidden="true">2</div>
            <div className="relative h-full flex flex-col justify-between gap-10">
              <span className="w-12 h-12 rounded-2xl border border-[rgb(var(--accent-rgb)/0.35)] bg-[rgb(var(--accent-rgb)/0.1)] text-[var(--accent-text)] flex items-center justify-center">
              ...
              <div>
                <strong className="block text-[54px] sm:text-[66px] font-extrabold leading-none tracking-[-0.07em] text-[var(--accent-text)]">2</strong>
                <h3 className="text-[14px] font-bold tracking-[0.08em] uppercase text-fg mt-3">{isBg ? 'Години гаранция' : 'Year warranty'}</h3>
                <p className="text-[13px] text-[var(--text-secondary)] mt-2 max-w-[34ch]">{isBg ? 'Локално обслужване без прехвърляне на отговорността.' : 'Local support without passing the responsibility elsewhere.'}</p>
              </div>
            </div>
          </div>
          ...
          <div className="about-reveal lg:col-span-5 surface-card rounded-[1.5rem] sm:rounded-[2rem] min-h-[250px] p-6 sm:p-8 flex flex-col justify-between gap-8">
            <div className="flex items-center justify-between gap-4">
              ...
              <span className="text-[10px] font-bold tracking-[0.14em] uppercase text-[var(--text-muted)]">Sofia // BG</span>
            </div>
            <div>
              ...
              <p className="text-[14px] text-[var(--text-secondary)] leading-relaxed mt-3">{isBg ? 'Поддръжка, диагностика и оригинални части на едно място.' : 'Maintenance, diagnostics and original parts in one place.'}</p>
            </div>
          </div>
              ...
              {journey.map(([number, title, text], index) => (
                <div key={number} className={`group py-4 md:px-7 first:pl-0 last:pr-0 ${index < journey.length - 1 ? 'border-b md:border-b-0 md:border-r border-fg/10' : ''}`}>
                  <div className="flex items-center gap-3 mb-5">
                    ...
                    <span className="h-px flex-1 bg-fg/10 group-hover:bg-[rgb(var(--accent-rgb)/0.4)] transition-colors duration-200" aria-hidden="true" />
                  </div>
                  <h3 className="text-[18px] font-bold tracking-[-0.025em] text-fg">{title}</h3>
                  <p className="text-[13px] text-[var(--text-secondary)] mt-2">{text}</p>
                </div>
              ))}
            </div>
          </div>
