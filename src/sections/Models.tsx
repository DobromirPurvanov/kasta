...
export default function Models({ showHeader = true }: ModelsProps) {
  ...
  return (
    <section
      id="models"
      ref={sectionRef}
      className={`relative bg-[var(--bg)] overflow-hidden ${showHeader ? 'section-pad' : 'py-14 sm:py-20 lg:py-24'}`}
    ...
    >
      ...
      <div className="section-shell relative">
          ...
          <div className="models-heading grid lg:grid-cols-[minmax(0,1fr)_minmax(300px,440px)] gap-6 lg:gap-14 items-end mb-9 sm:mb-12">
            <div>
              ...
              <h2 id="models-title" className="text-display text-fg uppercase text-[clamp(2.55rem,6vw,5.25rem)]">
                {isBg ? <>Избери своята<br /><span className="text-fg/35">машина.</span></> : <>Choose your<br /><span className="text-fg/35">machine.</span></>}
              </h2>
            </div>
            <div className="lg:pb-1">
              <p className="text-[15px] sm:text-[16px] text-fg/65 leading-[1.75] max-w-[52ch]">
                {isBg
        ...
        <div className="models-heading flex items-center justify-between gap-5 mb-5 sm:mb-6">
          ...
          <p className="shrink-0 text-[11px] font-bold tracking-[0.12em] uppercase text-fg/65" aria-live="polite" aria-atomic="true">
            ...
            return (
              ...
              >
                <div className={`relative media-tile overflow-hidden flex items-center justify-center p-4 sm:p-6 ${isFeatured ? 'aspect-[4/3] lg:aspect-auto lg:min-h-[490px]' : 'aspect-[4/3]'}`}>
                  ...
                  <span className="absolute top-4 right-4 z-10 inline-flex items-center gap-2 min-h-8 px-3 rounded-full border border-fg/10 bg-[var(--glass)] backdrop-blur-md text-[9px] font-bold tracking-[0.1em] uppercase text-fg/70">
                    <span className={`w-1.5 h-1.5 rounded-full ${isRoadLegal ? 'bg-emerald-500' : 'bg-[var(--accent)]'}`} aria-hidden="true" />
                    {isRoadLegal ? 'L1e' : isBg ? 'Офроуд' : 'Off road'}
                  </span>
                  <div className="absolute inset-x-[18%] bottom-[12%] h-[22%] bg-[rgb(var(--accent-rgb)/0.1)] blur-3xl rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-500" aria-hidden="true" />
                  <img
                    src={product.image}
                    alt={product.alt}
                    loading="lazy"
                    width="1100"
                    height="922"
                    className="relative w-full h-full object-contain transition-transform duration-500 ease-premium group-hover:scale-[1.055] group-hover:-translate-y-1 drop-shadow-2xl dark:brightness-125 dark:contrast-125"
                ...
                <div className="p-5 sm:p-6 lg:p-7 flex flex-col">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    {isFeatured && (
                      <span className="text-[9px] font-bold tracking-[0.13em] uppercase text-fg/65">{isBg ? 'Препоръчан' : 'Featured'}</span>
                    )}
                  </div>
                  ...
                  <p className="text-[13px] sm:text-[14px] leading-[1.65] text-fg/65 mt-3 line-clamp-2">
                  ...
                  <dl className="grid grid-cols-2 gap-2 mt-5">
                    <div className="rounded-xl bg-fg/[0.045] border border-fg/[0.06] px-3 py-3">
                      <dt className="text-[9px] font-bold tracking-[0.12em] uppercase text-fg/65">{isBg ? 'Скорост' : 'Speed'}</dt>
                      <dd className="text-[13px] font-bold text-fg mt-1">{topSpeed || '—'}</dd>
                    </div>
                    <div className="rounded-xl bg-fg/[0.045] border border-fg/[0.06] px-3 py-3">
                      <dt className="text-[9px] font-bold tracking-[0.12em] uppercase text-fg/65">{isBg ? 'Обхват' : 'Range'}</dt>
                  ...
                  <div className="flex items-end justify-between gap-4 mt-auto pt-6">
                    <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                      {product.originalPrice && <span className="text-[12px] text-fg/60 line-through">{product.originalPrice}</span>}
                      <span className="text-[21px] font-extrabold tracking-[-0.04em] text-fg">{product.price}</span>
                      <span className="text-[11px] text-fg/60">{product.priceBgn}</span>
                    </div>
                    <span className="w-11 h-11 shrink-0 rounded-full bg-fg/[0.06] border border-fg/10 flex items-center justify-center text-fg/75 group-hover:bg-[var(--accent)] group-hover:text-white group-hover:border-[var(--accent)] transition-all duration-300 group-hover:rotate-[-35deg]" aria-hidden="true">
