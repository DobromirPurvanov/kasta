      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
        {related.map((p) => (
          ...
          >
            ...
            <div className="p-5">
              <span className="text-[10px] font-semibold tracking-[0.15em] text-white/20 uppercase">
                {p.category}
              </span>
              ...
              <div className="flex items-baseline gap-2 mt-2">
                ...
                <span className="text-[12px] text-white/20">{p.priceBgn}</span>
              </div>
            </div>
          </div>
        ))}
    ...
    return (
      <div className="min-h-screen bg-[#0f0f0f] flex items-center justify-center pt-[72px]">
        <div className="text-center">
          <h1 className="text-xl font-light mb-4 text-white/40">
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-10 py-10 md:py-16">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-start">
          ...
          <div className="pd-anim" style={{ opacity: 0 }}>
            <span className="text-[11px] font-semibold tracking-[0.15em] text-white/20 uppercase">
              {product.category}
            </span>
            ...
            <div className="flex items-baseline gap-3 mb-8">
              {product.salePrice && (
                <span className="text-[18px] text-white/30 line-through">
                  {product.salePrice}
                </span>
              )}
              <span className="text-[32px] font-bold text-white">{product.price}</span>
              <span className="text-[14px] text-white/30">{product.priceBgn}</span>
            </div>

            <a
              href="mailto:office@kastaventures.com?subject=Inquiry: E RIDE PRO"
              className="btn-accent mb-10"
            >
            ...
            <div className="space-y-4">
              {(isBg ? product.descriptionBg : product.description)
                .split('\n\n')
                .map((para, idx) => (
                  <p key={idx} className="text-[15px] text-white/50 leading-[1.85]">
                    {para}
                  </p>
                ))}
            </div>
        ...
        <div className="max-w-[1200px] mx-auto px-6 md:px-10 text-center pd-anim" style={{ opacity: 0 }}>
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <a
              ...
              className="inline-flex items-center justify-center gap-2 bg-white text-[var(--accent)] px-8 py-3 rounded-full text-[13px] font-semibold uppercase tracking-wider hover:bg-white/90 transition-colors"
            >
              {isBg ? 'Изпрати запитване' : 'Send Inquiry'}
            </a>
            <a
              href="tel:+359887773733"
              className="inline-flex items-center justify-center gap-2 bg-transparent text-white px-8 py-3 rounded-full text-[13px] font-semibold uppercase tracking-wider border border-white/30 hover:bg-white/10 transition-colors"
            >
